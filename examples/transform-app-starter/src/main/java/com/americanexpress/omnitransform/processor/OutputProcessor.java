...
import com.americanexpress.omnitransform.App;
...
public class OutputProcessor implements Transformable, Serializable {
		...
		if ("J".equals(outputType)) {
			...
			if (transferIn != null && transferIn.getContent() != null) {
				...
				for (String key : out.getDependentIds()) {
					...
					for (String colName : out.getOutputPartitionKeys())
						colList.add(ds.col(colName));

					ds = ds.repartition(convertListToSeq(colList));

					if (out.getSaveMode().equalsIgnoreCase("APPEND"))
						ds.write().partitionBy(keyArr).mode(SaveMode.Append).parquet(out.getOutputFilePath());
					else
						ds.write().partitionBy(keyArr).mode(SaveMode.Overwrite).parquet(out.getOutputFilePath());
	...
	public Seq<org.apache.spark.sql.Column> convertListToSeq(List<org.apache.spark.sql.Column> inputList) {
